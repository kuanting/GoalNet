...
from model.train import train, val
from model.test import evaluate

# For BEV datasets with segmentation type feature_extract
...
class GoalNetTorch(nn.Module):
    def __init__(self, obs_len, pred_len, segmentation_model, feature_extract=False, feature_classes=6, encoder_channels=[], decoder_channels=[], waypoints=1):
        ...
        # feature_extract should be false and assign segmentation_model
        if feature_extract:
            print("[Warning] ConvNeXt feature extract module is being used instead of the segmentation model")
...
class GoalNet:
    ...
    def train(self, train_data, test_data, params, train_image_path, test_image_path, experiment_name, batch_size=8, num_goals=20, num_traj=1, device=None, dataset_name=None, val_data=None, val_image_path=None):
        """
        Train function
        ...
        if params['train_mode'] == 'scene':
            ...
            raise ValueError(f'{params["train_mode"]} train mode is not supported')
        ...
        #optimizer = torch.optim.NAdam(filter(lambda p: p.requires_grad, model.parameters()), lr=params["learning_rate"])
        ...
        for e in tqdm(range(params['num_epochs']), desc='Epoch'):
            ...
            val_ADE, val_FDE, _, _ = evaluate(model, test_loader, test_images, num_goals, num_traj, obs_len, pred_len, batch_size, device, input_template, 
                                            params, dataset_name, self.homo_mat, self.preprocessing_fn, seg_mask, use_TTST=False, use_CWS=False)
    ...
    def evaluate(self, data, params, image_path, batch_size=8, num_goals=20, num_traj=1, rounds=1, device=None, dataset_name=None):
        ...
        for e in tqdm(range(rounds), desc='Round'):
            test_ADE, test_FDE, test_MSE, KDE_NLL = evaluate(model, test_loader, test_images, num_goals, num_traj, obs_len, pred_len, batch_size, device, input_template,
    ...
    def validate(self, data, params, image_path, batch_size=8, num_goals=20, num_traj=1, rounds=1, device=None, dataset_name=None):
        """
        Fast val without TTST and CWS and calc the speed of model itself
        :param data: pd.df, val data
        :param params: dictionary with training hyperparameters
        :param image_path: str, filepath to val images
        :param batch_size: int, batch size
        ...
        params['bbox_wh_model'] = None # Won't evaluate bbox MSE in validate
        ...
        # Load val scene images
        val_images = create_images_dict(data, image_path=image_path, image_file=image_file_name, factor=params['resize'])
        
        val_dataset = SceneDataset(data, resize=params['resize'], total_len=total_len, splitby=splitby)
        val_loader = DataLoader(val_dataset, batch_size=loader_batch_size, collate_fn=collate_fn)
        ...
        ts = time.time()
        for e in tqdm(range(rounds), desc='Round'):
            val_ADE, val_FDE, _, _ = evaluate(model, val_loader, val_images, num_goals, num_traj, obs_len, pred_len, batch_size, device, input_template, 
                                        params, dataset_name, self.homo_mat, self.preprocessing_fn, seg_mask, use_TTST=False, use_CWS=False)
            #print(f'Round {e}: \nVal ADE: {val_ADE} \nVal FDE: {val_FDE}')
            
            self.eval_ADE.append(val_ADE)
            self.eval_FDE.append(val_FDE)
        
        te = time.time()
        print(f'Processing Speed: {round((te-ts)/ rounds / (len(data) / total_len) * 1000, 3)} ms / traj.')
        if params['matrics'] == 'ade':
            print(f'Average performance over {rounds} rounds: \nVal ADE: {sum(self.eval_ADE) / len(self.eval_ADE)} \nVal FDE: {sum(self.eval_FDE) / len(self.eval_FDE)}')
        elif params['matrics'] == 'mse':
            print(f'Average performance over {rounds} rounds: \nVal CMSE: {sum(self.eval_ADE) / len(self.eval_ADE)} \nVal CFMSE: {sum(self.eval_FDE) / len(self.eval_FDE)}')
